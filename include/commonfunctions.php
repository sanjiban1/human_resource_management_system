...
function checkTableName($shortTName )
{
	...
	if ("users" == $shortTName )
		return true;
	if ("employee" == $shortTName )
		return true;
	if ("branch" == $shortTName )
		return true;
	if ("task_assign" == $shortTName )
		return true;
	if ("loan" == $shortTName )
		return true;
	if ("notice" == $shortTName )
		return true;
	if ("payroll" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("employee");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("branch");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("task assign");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("loan");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("notice");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("payroll");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
{		
	if( $stype == "AVERAGE" )
	{
		...
		if( $sFormat == FORMAT_TIME ) {
			if( $value ) {
				...
				$value = ViewTimeField::getFormattedTotals( $field, round($value/$iNumberOfRows, 0), $pSet, $pageObject->pdfJsonMode() );
			}
		} else {
	...
	if( $stype == "TOTAL" || $stype == "SUM" )
	{
		if( $sFormat == FORMAT_TIME ) {
			include_once getabspath('classes/controls/ViewTimeField.php');
			$value = ViewTimeField::getFormattedTotals( $field, $value, $pSet, $pageObject->pdfJsonMode() );
		}
	}
...
{
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?34702")."\"></script>";
	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?34702")."\"></script>";
...
function getListOfSuggests( $sfields, $table, $whereClauses, $numberOfSuggests, $searchOpt, $searchFor, $searchField = '', $detailKeys = array() )
{
	...
	foreach( $sfields as $f )
	{
		...
		$dc = new DsCommand();
		$dc->filter = $fieldControl->getSearchCondition( $searchFor, $searchOpt );
		$dc->totals[] = array(
			"field" => $f,
			"total" => "distinct"
		);
...
function projectURL() {
	...
	return request_protocol() . "://" . $host . projectPath();
}
...
function getTableDataSource( $table, $connId ) {
	...
	if( GetTableURL( $table ) ) {
		$pSet = new ProjectSettings( $table );
		return getDataSource( $table, $pSet );
	}
	$dbTableInfo = DB::_getTableInfo( $table, $connId );
	if( $dbTableInfo ) {
		return new DataSourceDbTable( $table, $connection, $dbTableInfo );
	}
	return null;
}
...
function prepareHttpBody( $params ) {
	$data = array();
