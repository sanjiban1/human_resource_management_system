...
class Security
{
	...
	public static function setAllowedPages( $table, $type, $allowedPages )
	{
		...
		foreach( $allPages as $p ) {
			if( array_search( $p, $allowedPages ) === false )
				$pages[] = $p;
		}
	 ...
	 */
	public static function SelectCondition( $strRequestedPremission, $pSet )
	{
		//	not a project table
		if( !$pSet ) {
			return null;
		...
		if( strpos( $strPerm, $strRequestedPremission ) === false ) {
			return DataCondition::_False();
		}

   		$ownerid = @$_SESSION["_" . $pSet->table() . "_OwnerID"];
