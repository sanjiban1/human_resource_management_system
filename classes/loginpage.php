...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("users")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		// search by SpUserIdField field first, then by username

		$sqlHead = "select * from ".$this->connection->addTableWrappers( "users" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		foreach( $sql as $s ) {
			$data = $this->connection->query( $s )->fetchAssoc();
			if( $data )
				break;
		}
		...
		else if( $addNewUser )
		{
			...
			$data[ $cUserNameField ] = $info["id"];
			if( $userIdField )
				$data[ $userIdField ] = $info["id"];
			...
			$blobfields = array();
			$retval = DoInsertRecord( "users", $data, $blobfields, $this );
			if( !$retval )
				return false;
	...
	protected function assignBody()
	{
		...
		$this->body["end"] .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?34702")."\"></script>";
